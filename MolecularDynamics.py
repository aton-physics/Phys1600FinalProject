...
def jit_lennardJonesForce(N, L, x, y):
    ...
    for ii  in range(N):
        for jj in range(ii+1, N):
            ...
            if( dx > halfL): dx = dx - L
            if( dx < -halfL): dx = dx + L
            #dx = dx - int(dx * invL + (0.5 if dx >= 0.0 else -0.5)) * L

            dy = y[ii] - y[jj]
            if( dy > halfL): dy = dy - L
            if( dy < -halfL): dy = dy + L 
            #dy = dy - int(dx * invL + (0.5) if dy >= 0.0 else (-0.5)) * L
...
class MolecularDynamics:
    ...
    def evolve(self, time=10.0):
        ...
        self.xArray_unwrap = self.xArray_unwrap.tolist()
        ...
        for i in range(steps):
            ...
            if (i % self.sampleInterval == 0): # only calculate energy every sampleInterval steps to reduce load
                ...
                self.xArray_unwrap.append(self.x_unwrap)
                #does indeed give the right number of particles
        ...
        self.xArray_unwrap = np.array(self.xArray_unwrap)
...
# PLOTTING METHODS
    ...
    def pairCorrelation(self): 
        #i've fixed the bin width to be 0.01, by the way.
        #from all stored configurations, compute the pair correlation
        halfL = int(0.5 * self.L)
        binArray = np.zeros(halfL*100)
        configs = np.array(self.xArray[::1])
        numCfgs = len(configs)
        separation = np.array([])
        it = 0
        for n in range(numCfgs):
            index = n*self.N #gives location of the configuration in the r[i] arrays
            for i in range(self.N - 1):
                r_i = np.array([configs[n][2*i], configs[n][2*i+1]])
                for j in range(i+1, self.N):
                    #starts from i+1 because we don't want to double count.
                    r_j = np.array([configs[n][2*j], configs[n][2*j+1]])
                    separation = r_i - r_j
                    distance = np.sqrt(self.minimumImage(separation[0])**2 + self.minimumImage(separation[1])**2)
                    if distance > self.L/2: #don't look past half the box length (image problem)
                        continue
                    distance *= 100 #truncate past the hundredths place
                    distance = int(distance) #turn distance into an integer which is used to index histogram
                    binArray[distance] += 2
        binwidth = 0.01
        density = self.N / (self.L**2) #2-dimensions for now.
        constant = 3.14159*density
        print("and here!")
        for i in range(len(binArray)):
            rlower = i * binwidth
            rupper = rlower + binwidth
            nideal = constant * (rupper**2 - rlower**2)
            binArray[i] /= (self.N * nideal * numCfgs)
        return binArray

    def pairCorrelationHistogram(self):
        plt.figure(10)
        binArray = self.pairCorrelation()
        domain = np.arange(0,self.L/2,0.01)
        plt.plot(domain, binArray)
        plt.xlabel(r'r/[$\sigma$]')
        plt.ylabel("g(r)")

    def msd(self, time):
        #issue: coordinates are wrapped. need to unwrap. write that into the report
        #self.sampleInterval
        configs = np.array(self.xArray_unwrap[::1])
        trajectory = np.swapaxes(configs, 0, 1)
        #trajectory = np.zeros([self.N, np.zeros([int(time/sampleFreq)],[int(time/sampleFreq)])])
        d00 = np.zeros([self.N])
        d10 = np.zeros([self.N])
        msd = np.zeros(len(configs))
        for i in range(self.N):
            d10 = trajectory[2*i][0]
            d20 = trajectory[2*i+1][0]
        for n in range(len(configs)):    
            for i in range(self.N):
                d1 = trajectory[2*i][n]
                d2 = trajectory[2*i+1][n]
                arg = (d1 - d10)**2 + (d2 - d20)**2
                msd[n] += arg
            msd[n] /= self.N
        plt.figure(11)
        domain = np.arange(0, time, self.sampleInterval*self.dt)
        plt.plot(domain,msd)
        plt.xlabel(r't/[$\tau$]')
        plt.ylabel(r'[$\Delta r^2$](t)/[$\sigma^2$]')
                
    def velocityHistogram(self):
        vx = self.vArray[::2]
        vy = self.vArray[1::2]
        plt.figure(5)
        plt.hist(self.vArray, bins= 100, density=1)
...
def liquidsolid():
    ...
    md.evolve(time=0.1) # initial time evolution
    md.plotTrajectories(md.N)
    ax = plt.gca()
    ax.set_title('t = 0 $\\rightarrow$ 0.1')

    md.evolve(time=0.1) 
    md.resetStatistics() # remove transient behavior
    md.evolve(time=3.8) # second time evolution
    md.plotTrajectories(md.N)
    ax = plt.gca()
    ax.set_title('t = 0.2 $\\rightarrow$ 4')
    ...
    md.pairCorrelationHistogram()

    md.plotTemperature()    

    md.showPlots()

def d2_solid(Tstart = 10):

    md = MolecularDynamics(N=64, L=8, initialTemperature = Tstart, dt = 0.001, forceType = "lennardJones_jit") 
    md.rectangularLatticePositions()
    md.randomVelocities()
    md.evolve(time=10)
    for i in range(100):
...
def freezing(Tstart = 100):
    
    # instantiate object
    md = MolecularDynamics(N=100, L=8, initialTemperature=Tstart, dt = 0.0001, forceType = "lennardJones_jit") 
    ...
    T_text = ax.text(0.05, 0.92, ' ', transform=ax.transAxes, fontsize = 16)
